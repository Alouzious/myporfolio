...
pub async fn get_social_links(pool: web::Data<sqlx::PgPool>) -> HttpResponse {
    ...
    match result {
        ...
        Err(e) => {
            ...
            HttpResponse::InternalServerError().json(json!({"error": "Failed to fetch social links"}))
        }
    }
}
...
pub async fn create_social_link(
    ...
    match result {
        Ok(row) => {
            ...
            HttpResponse::Created().json(json!({"status": "success", "id": id}))
        }
        Err(e) => {
            eprintln!("Error creating social link: {:?}", e);
            HttpResponse::InternalServerError().json(json!({"error": "Failed to create social link"}))
        }
    }
}
    ...
    match result {
        Ok(res) => {
            if res.rows_affected() == 0 {
                HttpResponse::NotFound().json(json!({"error": "Social link not found"}))
            } else {
        ...
        Err(e) => {
            ...
            HttpResponse::InternalServerError().json(json!({"error": "Failed to delete social link"}))
        }
    }
}
...
pub fn social_links_routes(cfg: &mut web::ServiceConfig) {
    cfg.service(
        web::scope("/api")
            .route("/social-links", web::get().to(get_social_links))
