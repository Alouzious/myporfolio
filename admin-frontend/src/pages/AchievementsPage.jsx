...
const emptyForm = { title: '', description: '', date_achieved: '', certificate_url: '' };
...
export default function AchievementsPage() {
  ...
  async function load() {
    try {
      const data = await fetchApi('/api/achievements');
  ...
  async function handleCreate(e) {
    ...
    setSuccess('');
    try {
      await postApi('/api/achievements', form);
      setForm(emptyForm);
  ...
  return (
    <div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-lg p-6 mb-6">
        <h2 className="text-lg font-semibold text-white mb-4">Add Achievement</h2>
        <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="date" value={form.date_achieved} onChange={(e) => setForm({ ...form, date_achieved: e.target.value })} className="w-full px-4 py-2.5 bg-gray-800 border border-gray-700 rounded-lg text-white focus:outline-none focus:border-cyan-500" />
          </div>
          <input placeholder="Certificate URL" value={form.certificate_url} onChange={(e) => setForm({ ...form, certificate_url: e.target.value })} className="md:col-span-2 px-4 py-2.5 bg-gray-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-cyan-500" />
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-lg overflow-hidden">
        {loading ? (
          ...
          <p className="text-gray-400 text-center p-8">No achievements yet.</p>
        ) : (
            ...
            <tbody className="divide-y divide-gray-800">
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-gray-800/30">
                  ...
                  <td className="px-4 py-3 text-gray-400 hidden md:table-cell">{item.date_achieved}</td>
